...
from discriminator import AdvP2PDiscriminator
from utils import calc_gradient_penalty, plot_single_ecg, plot_multiple_ecg
...
def train(netG, netD, optimizerG, optimizerD, dataloader):
    ...
    for epoch in tqdm(range(opt.start_epoch + 1, opt.start_epoch + opt.num_epochs + 1)):
        ...
        for i, sample in tqdm(enumerate(dataloader, 0)):
            ...
            fake = autograd.Variable(netG(noise).data)
            D_fake = netD(fake)
            D_fake = D_fake.mean()
            D_fake.backward(one)
            ...
            if train_G_flag:
                ...
                noise = noise.to(device)
                # noise_Var = embedding_layer(noise_Var)
